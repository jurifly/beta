...
import { onAuthStateChanged, signOut as firebaseSignOut, createUserWithEmailAndPassword, signInWithEmailAndPassword as signInWithEmail, updateProfile as updateFirebaseProfile, sendPasswordResetEmail, GoogleAuthProvider, signInWithPopup } from 'firebase/auth';
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const createNewUserProfile = useCallback(async (firebaseUser: User, name: string): Promise<UserProfile> => {
    ...
    const newProfile: UserProfile = {
      ...
      companies: [],
      activeCompanyId: '',
      legalRegion: 'India', // Default region
      ...
      teamMembers: [{ id: firebaseUser.uid, name: name || 'Me', email: firebaseUser.email || '', role: 'Admin' }],
      invites: [],
  ...
  const fetchUserProfile = useCallback(async (firebaseUser: User) => {
    ...
    if (userDoc.exists()) {
      ...
      if (profile.onboardingCompleted === undefined) updatesToApply.onboardingCompleted = false;
      ...
      const newProfile = await createNewUserProfile(firebaseUser, firebaseUser.displayName || 'New User');
  ...
  const updateCompanyChecklistStatus = useCallback(async (
    ...
    if (!company) {
        console.error(`Company with ID ${companyId} not found in the current user's profile.`);
        toast({
            variant: "destructive",
            title: "Update Failed",
            description: "Could not find the specified company.",
        });
        return;
    }
  ...
  const signUpWithEmailAndPassword = async (email: string, pass: string, name: string) => {
      ...
      if (firebaseUser) {
        ...
        const newProfile = await createNewUserProfile(userData, name);
        setUserProfile(newProfile);
      }
  };
 ...
 const deductCredits = useCallback(async (amount: number): Promise<boolean> => {
  ...
  const value = { user, userProfile, loading, isPlanActive, notifications, isDevMode, setDevMode, updateUserProfile, updateCompanyChecklistStatus, deductCredits, applyAccessPass, signInWithEmailAndPassword, signUpWithEmailAndPassword, signOut, sendPasswordResetLink, saveChatHistory, getChatHistory, addNotification, markNotificationAsRead, markAllNotificationsAsRead, addFeedback, getPendingInvites, acceptInvite, sendCaInvite, sendClientInvite, checkForAcceptedInvites, sendTeamInvite, revokeTeamInvite, removeTeamMember };
